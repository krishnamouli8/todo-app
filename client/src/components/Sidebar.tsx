...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar }) => {
  ...
  useEffect(() => {
    // Get user data when sidebar opens
    if (isOpen) {
      const user = todoApi.getUserData();
      setUserData(user);
    }
  }, [isOpen]);
  ...
  // Debug log
  console.log('Display name:', displayName, 'Display email:', displayEmail);
      ...
      >
        <div className="flex flex-col h-full p-4">
          ...
          <div className="flex flex-col items-center mb-8 mt-4">
            ...
            <div className="w-24 h-24 rounded-full overflow-hidden mb-4 border-2 border-white/20 bg-purple-500 flex items-center justify-center">
            ...
            <p className="text-white/60 text-sm text-center px-2 break-all">
              {displayEmail}
            </p>
          </div>
